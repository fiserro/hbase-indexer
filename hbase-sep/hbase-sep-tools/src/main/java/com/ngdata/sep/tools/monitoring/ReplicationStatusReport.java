...
public class ReplicationStatusReport {
	...
	public static String toJson(ReplicationStatus replicationStatus) throws JsonGenerationException, 
		...
		ArrayNode peers = root.arrayNode();
		root.put("peers", peers);
		ArrayNode recoveredQueues = root.arrayNode();
		root.put("recoveredQueues", recoveredQueues);

		for (String peerId : sort(replicationStatus.getPeersAndRecoveredQueues())) {
			ArrayNode servers;
			if (replicationStatus.isRecoveredQueue(peerId)) {
				servers = recoveredQueues;
			} else {
				servers = peers;
			}
			for (String server : sort(replicationStatus.getServers(peerId))) {
				...
				ObjectNode serverNode = servers.objectNode();
				serverNode.put("server", server);
				serverNode.put("Hlog count", String.valueOf(status.getHLogCount()));
				serverNode.put("Hlog size", formatAsMB(status.getTotalHLogSize()));
				serverNode.put("Hlog progress", formatProgress(status.getProgressOnCurrentHLog()));
				serverNode.put("Age of last shipped Op", formatDuration(status.ageOfLastShippedOp));
				serverNode.put("Timestamp of last shipped Op", formatTimestamp(status.timestampOfLastShippedOp));
				serverNode.put("Selected peer count", formatInt(status.selectedPeerCount));
	...
	public static void printReport(ReplicationStatus replicationStatus, PrintStream out) {
		...
		out.format(columnFormat, "", "(incl. current)", "(excl. current)", "progress", "shipped op", "shipped op",
				"count");

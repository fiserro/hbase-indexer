...
public class SepModelImpl implements SepModel {
	...
	public void addSubscription(String name,
		...
		if (!addSubscriptionSilent(name, tableCfs)) {
			throw new IllegalStateException("There is already a subscription for name '" + name + "'.");
		}
	}

	@Override
	public boolean addSubscriptionSilent(String name,
			Map<TableName, ? extends Collection<String>> tableCfs) throws InterruptedException, KeeperException, IOException {
		...
		try {
			...
			UUID uuid = UUID.nameUUIDFromBytes(Bytes.toBytes(internalName)); // always gives the same uuid for the same
																				// name
			ZkUtil.createPath(zk, basePath + "/hbaseid", Bytes.toBytes(uuid.toString()));
			ZkUtil.createPath(zk, basePath + "/rs");

			ReplicationPeerConfig config = new ReplicationPeerConfig();
			config.setClusterKey(zkQuorumString + ":" + zkClientPort + ":" + basePath);
			try {
				replicationAdmin.addPeer(internalName, config, tableCfs);
				...
				// HBase 0.95+ throws at least one extra exception: ReplicationException which we convert into
				// IOException

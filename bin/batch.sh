...
if [ $# -eq 0 ]
  then
    echo "Pass 1 argument: <config.xml>"
    exit 1
fi
...
echo $cp
echo -------------------------------------------------
HADOOP_CLASSPATH=$(hbase mapredcp 2>&1 | tail -1):/opt/hbase/conf:`pwd`/lib/* yarn jar `pwd`/tools/hbase-indexer-mr-1.7-SNAPSHOT-job.jar \
